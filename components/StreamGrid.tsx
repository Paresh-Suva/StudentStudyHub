...
const CircuitBackground = ({ color = "#3b82f6" }: { color?: string }) => {
    return (
        <div className="absolute inset-0 z-0 overflow-hidden opacity-20 transition-opacity duration-300 md:group-hover:opacity-40 max-md:opacity-40">
                            ...
                            <Link key={stream.id} href={stream.href} className="block h-full cursor-pointer">
                                ...
                                >
                                    <div className={clsx(
                                        ...
                                        "border border-white/10 md:group-hover:border-[var(--stream-color)] md:group-hover:shadow-[0_0_30px_-5px_var(--shadow-color)]", // Desktop Hover
                                        "max-md:border-[var(--stream-color)] max-md:shadow-[0_0_30px_-5px_var(--shadow-color)]", // Mobile Always Active
                                        "rounded-[2rem]"
                                    )} />
                                    ...
                                    <div className="absolute right-8 top-8 z-20">
                                        <div className="flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white transition-all duration-300 md:group-hover:bg-[var(--stream-color)] md:group-hover:text-white md:group-hover:scale-110 max-md:bg-[var(--stream-color)] max-md:scale-110">
                                            <ArrowRight className="h-5 w-5 -rotate-45 transition-transform duration-300 md:group-hover:rotate-0 max-md:rotate-0" />
                                        </div>
                                    </div>

                                    {/* Main Icon (Centered) */}
                                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 transition-transform duration-500 md:group-hover:scale-110 max-md:scale-110">
                                        <Icon
                                            className="h-32 w-32 stroke-1 transition-colors duration-300 text-zinc-700 md:group-hover:text-[var(--stream-color)] max-md:text-[var(--stream-color)]"
                                            style={{
                                    ...
                                    <div className="relative z-20 mt-auto">
                                        <h2 className="mb-2 text-4xl font-bold tracking-tight text-white transition-colors duration-300 md:group-hover:text-[var(--stream-color)] md:text-5xl max-md:text-[var(--stream-color)]">
                                            {stream.title}
                                        </h2>
                                        <p className="max-w-sm text-lg font-medium text-zinc-500 md:group-hover:text-zinc-300">
                                            {stream.subtext}
                                        </p>
                                    </div>
                                </div>
